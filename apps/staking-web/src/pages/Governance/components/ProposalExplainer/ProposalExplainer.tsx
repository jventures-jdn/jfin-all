        ...
        <div>
            ...
            >
                ...
                <Descriptions.Item key="status" label="Status">
                    <span>{renderStatus(event.status)}</span>
                </Descriptions.Item>

                <Descriptions.Item key="proposer" label="Proposer Address">
                    {event.proposer}
                </Descriptions.Item>

                <Descriptions.Item key="startBlock" label="Start Block">
                    {event.startBlock}
                </Descriptions.Item>

                <Descriptions.Item key="endBlock" label="End Block">
                    {event.endBlock}
                </Descriptions.Item>

                <Descriptions.Item key="description" label="Description">
                    {event.desc}
                </Descriptions.Item>
            </Descriptions>
            ...
            <Table
                columns={[
                    ...
                    { title: 'Block', dataIndex: 'blockNumber' },
                    {
                        title: 'Weight',
                        dataIndex: 'weight',
                        render: (value: BigNumber) => value.toString(10),
                    },
                ]}
            ...
            <Descriptions bordered column={1} layout="horizontal" size="small" title="Actions">
                {event.targets.map((value: any, index: number) => (
                    <Descriptions.Item key={value} label={`${value} (${event.values[index]} wei)`}>
                        {event.inputs[index]}
                    </Descriptions.Item>
                ))}
            </Descriptions>
