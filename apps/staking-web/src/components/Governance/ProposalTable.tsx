    ...
    const columns: ColumnProps<Proposal>[] = [
        {
            ...
            render: (proposal: Proposal) => (
                <span style={{ textTransform: 'capitalize' }}>{proposal.args.description}</span>
            ),
        },
        {
            title: 'Proposal From',
            render: (v: Proposal) => (
                <div className="items-center column-validator">
                    <img
                        src={VALIDATOR_WALLETS[v.args.proposer as string]?.image || defaultImage}
                        alt={VALIDATOR_WALLETS[v.args.proposer as string]?.name || v.args.proposer}
                        style={{
                            width: '30px',
                            height: '30px',
                            borderRadius: '50%',
                    ...
                    <div>
                        <span>
                            {VALIDATOR_WALLETS[v.args.proposer as string]?.name || v.args.proposer}
                        </span>
            ...
            render: (proposal: Proposal) => (
                <>
                    <a
                        ...
                        }jfinchain.com/block/${Number(proposal.args.startBlock)}/transactions`}
                        target="_blank"
                        rel="noreferrer"
                    >
                    ...
                    <a
                        href={`https://exp.${
                            EXPECT_CHAIN.chainName === 'JFIN' ? '' : 'testnet.'
                        }jfinchain.com/block/${Number(proposal.args.endBlock)}/transactions`}
                        target="_blank"
                        rel="noreferrer"
                    >
